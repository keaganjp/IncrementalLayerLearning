...
if __name__ == '__main__':
    ...
    filename = args.dataset + '_memtest_' + args.net + '.txt'
    f = open(filename, "w")
    f.write('Testing!\n')
    ...
    for epoch in range(1, settings.EPOCH + 1):
        ...
        f.write('epoch {} with accuracy: {:.3f}\n'.format(epoch, acc*100.0))
        ...
        if not epoch % settings.SAVE_EPOCH:
            ...
            print('saving weights file to {}'.format(weights_path))
            torch.save(net.state_dict(), weights_path)
    final_test_acc = eval_training()
    memory_usage = torch.cuda.memory_stats()["allocated_bytes.all.peak"]
    print("Mem stat")
    print(memory_usage)
    f.write('Final testing accuracy: {:.3f}\n'.format(final_test_acc*100.0))
    f.close()
    writer.close()
